...
class ScopedExpoNotificationPresentationModule(
  ...
  override fun createNotificationRequest(
    ...
    return ScopedNotificationRequest(identifier, content, trigger, experienceScopeKey)
  }
      ...
      object : ResultReceiver(null) {
        override fun onReceiveResult(resultCode: Int, resultData: Bundle) {
          ...
          if (resultCode == NotificationsService.SUCCESS_CODE && notifications != null) {
            ...
            if (notification == null || !scopedNotificationsUtils.shouldHandleNotification(
                notification,
                experienceKey
              )
            ) {
  ...
  private fun findNotification(
    ...
    for (notification in notifications) {
      if (notification.notificationRequest.identifier == identifier) {
        return notification
      }
    }
    return null
